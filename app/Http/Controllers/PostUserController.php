...
class PostUserController extends Controller
{
      public function index(){
        ...
        $posts = Post::with('user')->where('user_id','=',$user->id)->where('is_delete','=',0)->get();
        $success = [
    ...
    public function show($id){
        $user = User::select('users.*')->find(auth()->guard('user')->user()->id);
          $posts = Post::with('user')->where('user_id','=',$user->id)->where('is_delete','=',0)->where('id',$id)->firstOrFail();
            $success = [
    ...
    public function update(Request $request, $id){
        $validator = Validator::make($request->all(), [
                ...
                $validationpost = Post::where('id',$id)->firstOrFail();


                if($validationpost->user_id == $user->id){
                    ...
                    $fail = [
                        "message" => 'update data anda gagal karena usernya tidak sesuai'
                    ];
